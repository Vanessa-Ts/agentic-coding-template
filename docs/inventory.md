...
Generated 2026-06-05 during Phase 3. Every `.claude/` file has exactly one row.
...
| `agents/quality-reviewer.md` | project-internal | proprietary | — | ✓ | — | FastAPI-specific checklist (async, `response_model`, `Depends`) limits direct reuse. Wrap: replace checklist items for a different framework. Model: `claude-haiku-4-5-20251001`. |
...
| `commands/review.md` | project-internal | proprietary | ✓ | — | — | Runs quality-reviewer and security-reviewer in parallel; 2-iteration self-correction loop; no project-specific logic. Reuse as-is alongside the two reviewer agents. |
